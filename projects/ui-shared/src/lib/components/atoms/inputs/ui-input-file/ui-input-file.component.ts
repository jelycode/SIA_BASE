...
export class UiInputFileComponent implements ControlValueAccessor {
  ...
  triggerFileInput(): void {
    if (!this.disabled && !this.readonly()) this.fileInput.nativeElement.click();
  }
  ...
  handleFileChange(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      this.onChange(this.multiple() ? this._files : this._files[0]);
    }
  }
  ...
  getErrorMessage(): string { return 'Este campo es requerido'; }
